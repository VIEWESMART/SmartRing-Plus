...
#define ALPHA                       0.85f        /*!< Weight of gyroscope */
...
int icm42670_imu_write(inv_imu_serif_t* serif, const uint8_t reg_start_addr, const uint8_t *data_buf, const uint8_t data_len)
{
    icm42670_dev_t *sens = (icm42670_dev_t *) serif->context;
    //icm42670_dev_t *sens = (icm42670_dev_t *) sensor;
    esp_err_t  ret;
...
int icm42670_imu_read(inv_imu_serif_t* serif, const uint8_t reg_start_addr, uint8_t *data_buf, const uint8_t data_len)
{
    icm42670_dev_t *sens = (icm42670_dev_t *) serif->context;
    //icm42670_dev_t *sens = (icm42670_dev_t *) sensor;
    uint8_t reg_buff[] = {reg_start_addr};
    ...
    /* Write register number and read data */
    i2c_master_write_read_device(sens->bus, sens->dev_addr, reg_buff, sizeof(reg_buff), data_buf, data_len, 1000 / portTICK_PERIOD_MS);
    return 0;
}
esp_err_t icm42670_calibrate_gyro(icm42670_handle_t sensor, const icm42670_value_t *const acce_value,
                                        icm42670_value_t *const gyro_value_cal)
{ 
    float acce_angle[2];
    float gyro_angle[2];
    gyro_value_cal->x = 0;
    ...
    for(int i = 0; i < 1000; i++) {
        icm42670_value_t value_before;
        icm42670_get_gyro_value(sensor,&value_before);
    ...
    ESP_LOGI(TAG, "icm42670_calibrate_gyro x = %f, y = %f, z = %f",gyro_value_cal->x,gyro_value_cal->y,gyro_value_cal->z);
    return ESP_OK;
}
